...
The exact `PRE_DEPLOY` package must lock one Studio account as both deployer and initial upgrader before any deployment transaction. The constructor registers the runtime deployment sender in `gl.storage.Root.get().upgraders`; finalized `get_upgrader()` readback must match the locked public address.
